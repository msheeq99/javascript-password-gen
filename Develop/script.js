...
function generatePassword() {
  ...
  if (passwordUpperCase === "yes") {
    charsMixed = charsMixed.concat(passwordUpperCase);
  ...
  if (passwordLowerCase === "yes") {
    charsMixed = charsMixed.concat(passwordLowerCase);
  ...
  if (passwordSymbols === "yes") {
    charsMixed = charsMixed.concat(passwordSymbols);
  ...
  if (passwordNumbers === "yes") {
    charsMixed = charsMixed.concat(passwordNumbers);
...
console.log(charsMixed);

var password = ""

for (let i = 0, n = charsMixed.length; i <parseInt(pw); i++) {
  password += charsMixed[Math.floor.apply(Math.random() * n)]
} 
